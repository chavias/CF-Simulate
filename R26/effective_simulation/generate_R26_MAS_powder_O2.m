function result = generate_R26_MAS_powder_O2(repetitions)
...
for nur_index=1:length(nur_list) % nur loop
    ...
    h2 = calc_all_h2(nur_list,nur_index,nu_m,T,npoints,nu_eff);
                        ...
                        for p2=-2:2
                            ...
                            for m1_index= 1:(2*npoints-1) % coefficients A index
                                for m2_index= 1:(2*npoints-1) % coefficients B index
                                    commu = comm(:,:,m1_index,m2_index,p1+3,p2+3);
                                    Ham2 = Q*commu;
...
function h2 = calc_all_h2(nur_list,nur_index,nu_m,T,npoints,nu_eff)
            ...
            for n1=-2:1:2 % dipolar coupling A
                for p1=-2:2
                    for p2=-2:2
                        ...
                        nuA = n1*nur_list(nur_index)+m1*nu_m+p1*nu_eff; % Hz linear
                        % resonance condition B (m1,m2)
                        nuB = n2*nur_list(nur_index)+m2*nu_m+p2*nu_eff; % Hz linear
                        % second-order basis function
