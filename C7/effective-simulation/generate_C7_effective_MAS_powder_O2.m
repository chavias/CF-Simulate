function  result = generate_C7_effective_MAS_powder_O2(repetitions)
...
% calc all commutator
comm = calc_all_comm(Hn,npoints);

parfor nur_index=1:length(nur_list) % nur loop
    %tic;

    h1 = calc_all_h1(nu_m,npoints,nur_list,nur_index,T);
    h2 = calc_all_h2(nur_list,nur_index,nu_m,T,npoints);
    ...
    for powder_index=1:length(beta) % powder loop
        ...
        for n2=-2:1:2 % dipolar coupling B
            for n1=-2:1:2 % dipolar coupling A
                ...
                Q = -1/2*wn(n1+3,powder_index)*wn(n2+3,powder_index);
                for m1_index= 1:(2*npoints-1) % coefficients A index
                    for m2_index= 1:(2*npoints-1) % coefficients B index
                        commu = comm(:,:,m1_index,m2_index);
                        Ham2 = Q*commu;
                        % second-order basis function
                        Ham2 = Ham2 .* h2(n1+3,n2+3,m1_index,m2_index,nur_index);
                        HamA = HamA + Ham2;
                    end
                end
            end
...
function h2 = calc_all_h2(nur_list,nur_index,nu_m,T,npoints)
...
function h1 = calc_all_h1(nu_m,npoints,nur_list,nur_index,T)
...
%                 for m2_index= 1:(2*npoints-1) % coefficients B index
%                     m2 = m2_index-npoints;
%                     comm = commutator(Hn(:,:,m1_index),Hn(:,:,m2_index));
%                     for n2=-2:1:2 % dipolar coupling B
%                         for n1=-2:1:2 % dipolar coupling A
%                             % resonance condition A (n1,n2)
%                             nuA = n1*nur_list(nur_index)+m1*nu_m; % Hz linear
