...
if __name__=='__main__':
    ...
    time_mod = 0.1e-3 # sec determind by shapefile
    n_1rep =  500 # points determinde by shapefile
    # nu_r = np.linspace(20e3,150e3,1001) # Hz
    nu_r = 100e3
    sweep = np.linspace(40e3,60e3,2001)
    reps = [10] # 1 means pulse scheme is applied once
    for rep in reps:
        print(f'============================== rep = {rep} ==============================')
        for item in sweep:
            seq,step,n_max,n_sample,nu_r = calc_sim_param(\
                                                      rep*time_mod,rep*n_1rep,nu_r,seq='CW-500')
